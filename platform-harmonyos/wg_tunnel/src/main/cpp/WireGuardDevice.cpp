...
//        napi_reference_unref(env, client->wrapper_, &refCount);
    ...
    napi_value Start(napi_env env, napi_callback_info info) {
        ...
        try {
            ...
            if (device) {
                ...
                auto currentClient = device->device;
                if (currentClient) {
                    currentClient->start(fd);
    ...
    void GetConnectConfig(napi_env env, napi_value arg, WireGuard::DeviceRegisterConfig &config) {
        napi_value nvClient;
        napi_get_named_property(env, arg, "client", &nvClient);
        GetClientConfig(env, nvClient, config.client);
    ...
    void GetClientConfig(napi_env env, napi_value arg, WireGuard::DeviceConfig &device) {
        // 获取 privateKey
