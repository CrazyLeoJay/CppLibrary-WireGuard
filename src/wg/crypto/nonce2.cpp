...
namespace WireGuard {
    std::shared_ptr<KeyPair> crypto2::NOISE::makeKeyPair(const bool &iAmInitiator) const {
    ...
    void crypto2::NOISEReceive::init(const PrivateKey &local_private) {
    ...
    void crypto2::NOISEReceive::decodeCheckHandshakeInitiation(const MessageInitiation &msg) {
    ...
    MessageResponse crypto2::NOISEReceive::createHandshakeResponse(const uint32_t &senderIndex) {
        std::lock_guard<std::mutex> lock(_noiseMutex);
    ...
    void crypto2::NOISESend::init(const PrivateKey &local_private, const PublicKey &remote_public) {
        state = NOISEState::none;
        this->local_private = local_private;
    ...
    MessageInitiation crypto2::NOISESend::encodeHandshakeInitiation(uint32_t senderIndex) {
        std::lock_guard<std::mutex> lock(_noiseMutex);
    ...
    void crypto2::NOISESend::verifyHandshakeInitiationResponse(const MessageResponse &msg) {
        std::lock_guard<std::mutex> lock(_noiseMutex);
